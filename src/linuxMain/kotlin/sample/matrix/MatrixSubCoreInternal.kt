...
open class MatrixSubCoreInternal<T>: Math, AssignableIterable<T?> {
    // constructors
    constructor() : super() {
    ...
    open fun resizeAndAssign(size: Int, assigned: () -> T) = when {
        size == 0 -> vector.clear()
        size > vector.size -> while(size > vector.size) vector.add(assigned())
        else -> while(size < vector.size) vector.removeAt(vector.lastIndex)
    }
    open fun addDimension(default: T) = vector.add(default)
    open fun removeDimension(dimension: Int) = vector.removeAt(dimension)
    open fun addAll(vectorCore: MatrixSubCoreInternal<T>) = vector.addAll(vectorCore.vector)
    // vector indexing
    open operator fun get(index: Int) = vector.get(index)
    open operator fun set(index: Int, value: T) {
        vector[index] = value
    }
    open fun clone() = MatrixSubCoreInternal<T>().also {
        it.addAll(this)
    }
